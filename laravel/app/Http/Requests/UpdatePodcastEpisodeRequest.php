...
class UpdatePodcastEpisodeRequest extends FormRequest
{
    ...
    {
        abort_if(Gate::denies('podcast_episode_edit'), Response::HTTP_FORBIDDEN, '403 Forbidden');
    ...
    public function rules()
    {
        return [
            ...
            'episode_title'                    => [
                ...
                'max:255',
                'required',
                'unique:podcast_episodes,episode_title,' . request()->route('podcast_episode')->id,
            ],
            'episode_slug'                     => [
                'min:10',
                'max:255',
                'required',
                'unique:podcast_episodes,episode_slug,' . request()->route('podcast_episode')->id,
            ],
